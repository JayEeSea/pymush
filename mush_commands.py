def handle_command(command, client_socket, clients, channels, channel_config_file, player_rooms, rooms, help_file='help_topics.txt'):
    if command.startswith('/quit'):
        ...
        client_socket.send(f'Channel "{display_name}" added successfully.'.encode('utf-8'))
    elif command.startswith('/listchannels'):
        # List all chat channels
        channel_list = [f'{name} ({display_name})' for name, display_name in channels.items()]
        client_socket.send(f'Available channels: {", ".join(channel_list)}'.encode('utf-8'))
        ...
        client_socket.send(f'Channel "{channel_name}" removed successfully.'.encode('utf-8'))
    elif command.startswith('/look'):
        # Look around the current room
        room_id = player_rooms.get(client_socket, "1")
        room = rooms.get(room_id)
        if room:
            client_socket.send(f'You are in {room["name"]}. {room["description"]}'.encode('utf-8'))
            if room["exits"]:
                exits = ', '.join(room["exits"].keys())
                client_socket.send(f'Exits: {exits}'.encode('utf-8'))
        else:
        ...
        if current_room and direction in current_room["exits"]:
            ...
            client_socket.send(f'You move {direction} and arrive in {new_room["name"]}. {new_room["description"]}'.encode('utf-8'))
        else:
